...
function valid_email($str)
{
    ...
    $addr_spec='/^[^0-9][a-zA-Z0-9_]+([.][a-zA-Z0-9_]+)*@\[?([\d\w\.-]+)]?$/';
...
function birthday_all($birthday_all) // 誕生日が現在日時よりも前ではないか
{
    return $birthday_all <= date('Y-n-j') ? true : false ;
}

function m_pref_check($data)
{
    $CI =& get_instance();
    $CI->load->model('Welcome_model');
    $m_pref = $CI->welcome_model->get_m_pref();
    return !(empty($m_pref[$data-1])) ? true : false;
}
